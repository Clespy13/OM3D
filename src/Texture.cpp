...
namespace OM3D
{
    ...
    Texture Texture::empty_cubemap_array(u32 size, u32 layers,
                                         ImageFormat format, u32 mipmaps)
    {
        Texture cube;
        {
            cube._handle =
                GLHandle(create_texture_handle(GL_TEXTURE_CUBE_MAP_ARRAY));
            cube._texture_type = GL_TEXTURE_CUBE_MAP_ARRAY;
            cube._size = glm::uvec2(size);
            cube._format = format;
        }
        ...
        glTextureStorage3D(
            cube._handle.get(), std::min(mipmaps, mip_levels(cube._size)),
            gl_format.internal_format, cube._size.x, cube._size.y, layers * 6);
    ...
    std::vector<unsigned char> Texture::data() const
    {
        ...
        std::vector<unsigned char> res(_size.x * _size.y
                                       * image_format_to_size(_format));
        glBindTexture(GL_TEXTURE_2D, _handle.get());
        glGetTexImage(GL_TEXTURE_2D, 0, image_format_to_gl(_format).format,
